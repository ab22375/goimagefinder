...
func (l *CR3ExiftoolLoader) LoadImage(path string) (image.Image, error) {
	...
	if len(fileInfos) == 0 {
		return nil, fmt.Errorf("no metadata extracted")
	}
	...
	return nil, fmt.Errorf("failed to extract any preview from CR3 file")
}
...
func validateImageFile(path string) bool {
	...
	// Try to read with imaging library
	_, err := imaging.Open(path)
	return err == nil
}
